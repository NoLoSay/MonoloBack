...
export class UtilsService {
  ...
  private dateRegex: RegExp =
    /(?:(\d{1,2})([/\-. ]?))?(\d{1,2}|\b(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?|janv(?:ier)?|fev(?:rier)?|mar(?:s)?|avr(?:il)?|mai|juin|juil(?:let)?|aou(?:t)?|sep(?:tembre)?|oct(?:obre)?|nov(?:embre)?|dec(?:embre)?)\b)?([/. ]?)((?:-?\d+))/gi;
