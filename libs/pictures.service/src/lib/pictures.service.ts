...
import { Picture, PrismaBaseService } from '@noloback/prisma-client-base';
...
export class PicturesService {
  ...
  async createPicture(picturePath: string): Promise<Picture> {
    try {
      ...
      const uploadThingPictureUrl = await this.uploadthingService.uploadFromUrl(
        updatedPicture.hostingUrl,
        path.extname(
          updatedPicture.localPath ? updatedPicture.localPath : 'default.png',
        ),
      );
      ...
      LoggerService.log(
        ...
        'PicturesService.createPicture',
        error,
        `Error creating picture: ${error.message}`,
      );
  ...
  async getPicture(uuid: string) {
    ...
    if (!picture.localPath) {
      throw new InternalServerErrorException(
        `Error while getting picture with UUID ${uuid} not found`,
      );
    }
