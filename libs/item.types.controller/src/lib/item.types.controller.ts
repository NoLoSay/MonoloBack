...
export class ItemTypesController {
  ...
  async findAll (
    ...
    @Query('createdAt_lte') createdAtLte?: string | undefined
      ...
      const itemTypes = await this.itemTypesService.findAll(request.user.activeProfile.role, new FiltersGetMany(firstElem, lastElem, sort, order, ['id', 'categoryId', 'name', 'createdAt']), itemCategoryId, nameStart, createdAtGte, createdAtLte)
        return res
      .set({
